...
*Word count target: ~ 400. Codex Round 6 unsafe-claim ban list is enforced (no "fully validated", "Tier 2 completed", "best-of-9 operational", or "exact identical scores"). Remaining placeholders for the camera-ready submission: GitHub repository URL (corresponding author has completed OAuth; team-lead has proposed `dad-protocol` as the public repo name — pending creation), Zenodo DOI (mint at acceptance of this protocol manuscript), submission date, exact iScience figure / supplementary identifier for the primary-paper Tier 1 ground truth (corresponding-author verification); see `Publication/submission_metadata/PLACEHOLDER_RESOLUTION_GUIDE.md`.*
